...
static void update_block_bitmap(void) {
    ...
    fwrite(bitbuf2, BLOCK_SIZE, 1, fp);
    fwrite(bitbuf3, BLOCK_SIZE, 1, fp);
    fflush(stdin);
    //while((c=getchar())!='\n' && c!=EOF);
}
...
static void reload_block_bitmap(void) {
    ...
    fread(bitbuf2, BLOCK_SIZE, 1, fp);
    fread(bitbuf3, BLOCK_SIZE, 1, fp);
}
 ...
 * 分配一个数据块，返回数据块号
 */
static int alloc_block(int mode) {
    unsigned short cur = last_alloc_block;
    unsigned char con = 128;
    int flag = 0;
    reload_block_bitmap();
    if (mode == 0 || mode == 1) {
        //Free to Transparent or Allocated
        if (gdt[0].bg_free_blocks_count == 0) {
            printf("there is no free block to be allocated!\n");
            return (0);
        }
        cur /= 8;
    ...
    if (mode == 3) {
        cur /= 8;
        while (bitbuf[cur] == 255 && bitbuf2[cur] == 0 && bitbuf3[cur] == 255) {
            if (cur == 511)
                cur = 0;
            else
                cur++;
 ...
 * 并加在bn到dir中
 */
static unsigned short reserch_file(char tmp[9], int file_type, unsigned short *inode_num, unsigned short *block_num,
 ...
 * 删除一个block
 */
static void remove_block(unsigned short del_num, int mode) {
    unsigned short tmp;
    tmp = del_num / 8;
    reload_block_bitmap();
    switch (del_num % 8)//更新block位图
    {
        case 0:
            if (mode == 0) {
                ...
                bitbuf2[tmp] = bitbuf2[tmp] & 127;
                bitbuf3[tmp] = bitbuf3[tmp] & 127;
            }
            if (mode == 1) {
                bitbuf3[tmp] = bitbuf3[tmp] & 127;
            }
            if (mode == 2) {
                bitbuf[tmp] = bitbuf[tmp] | 128;
                bitbuf2[tmp] = bitbuf2[tmp] & 127;
                bitbuf3[tmp] = bitbuf3[tmp] & 127;
            }
            break;//01111111
        case 1:
            ...
            bitbuf2[tmp] = bitbuf2[tmp] & 191;
            bitbuf3[tmp] = bitbuf3[tmp] & 191;
            if (mode == 1) {
                bitbuf3[tmp] = bitbuf3[tmp] & 191;
            }
            if (mode == 2) {
                bitbuf[tmp] = bitbuf[tmp] | 64;
                bitbuf2[tmp] = bitbuf2[tmp] & 191;
                bitbuf3[tmp] = bitbuf3[tmp] & 191;
            }
            break;//10111111
        case 2:
            ...
            bitbuf2[tmp] = bitbuf2[tmp] & 223;
            bitbuf3[tmp] = bitbuf3[tmp] & 223;
            if (mode == 1) {
                bitbuf3[tmp] = bitbuf3[tmp] & 223;
            }
            if (mode == 2) {
                bitbuf[tmp] = bitbuf[tmp] | 32;
                bitbuf2[tmp] = bitbuf2[tmp] & 223;
                bitbuf3[tmp] = bitbuf3[tmp] & 223;
            }
            break;//11011111
        case 3:
            ...
            if (mode == 1) {
                bitbuf3[tmp] = bitbuf3[tmp] & 239;
            }
            if (mode == 2) {
                bitbuf[tmp] = bitbuf[tmp] | 16;
                bitbuf2[tmp] = bitbuf2[tmp] & 239;
                bitbuf3[tmp] = bitbuf3[tmp] & 239;
            }
            break;
        case 4:
            ...
            if (mode == 1) {
                bitbuf3[tmp] = bitbuf3[tmp] & 247;
            }
            if (mode == 2) {
                bitbuf[tmp] = bitbuf[tmp] | 8;
                bitbuf2[tmp] = bitbuf2[tmp] & 247;
                bitbuf3[tmp] = bitbuf3[tmp] & 247;
            }
            break;
        case 5:
            ...
            if (mode == 1) {
                bitbuf3[tmp] = bitbuf3[tmp] & 251;
            }
            if (mode == 2) {
                bitbuf[tmp] = bitbuf[tmp] | 4;
                bitbuf2[tmp] = bitbuf2[tmp] & 251;
                bitbuf3[tmp] = bitbuf3[tmp] & 251;
            }
            break;
        case 6:
            ...
            if (mode == 1) {
                bitbuf3[tmp] = bitbuf3[tmp] & 253;
            }
            if (mode == 2) {
                bitbuf[tmp] = bitbuf[tmp] | 2;
                bitbuf2[tmp] = bitbuf2[tmp] & 253;
                bitbuf3[tmp] = bitbuf3[tmp] & 253;
            }
            break;
        case 7:
            ...
            if (mode == 1) {
                bitbuf3[tmp] = bitbuf3[tmp] & 254;
            }
            if (mode == 2) {
                bitbuf[tmp] = bitbuf[tmp] | 1;
                bitbuf2[tmp] = bitbuf2[tmp] & 254;
                bitbuf3[tmp] = bitbuf3[tmp] & 254;
            }
            break;
    }
